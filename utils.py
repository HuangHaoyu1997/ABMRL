...
def get_coord_railway_station(file=r'C:\Users\44670\Documents\GitHub\ABMRL\地铁站点.xls'):
    '''
    從file中读取地鐵站點的經緯度坐標，再將其轉化為圖像index
...
def rent_price(file=r'./data/20年房租coord.pkl'):
    '''
    计算房租数据在图像矩阵上的index
    '''
    max_x = 21290
    ...
    with open(file,'rb') as f:
        rent_coord_2020 = pickle.load(f)
    
    rent_coord = []
    for x,y in rent_coord_2020:
        coord = cal_coord([x,y])
        print(coord)
        if coord[0]<0 or coord[0]>max_x or coord[1]<0 or coord[1]>max_y:
            continue
        rent_coord.append(coord)
    with open('./data/20年房租index.pkl','wb') as f:
        pickle.dump(rent_coord,f)
